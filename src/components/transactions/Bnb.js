...
class Bnb extends Component {
    ...
    async componentDidMount() {
        ...
        if(this.props.type=="Token"){
            axios.get("https://api.bscscan.com/api?module=account&action=tokentx&contractaddress="+contractaddress+"&address="+address+"&page=1&offset=100&startblock=0&endblock=2007025780&sort=desc&apikey=7IEVKBDRZ9IQZGX2A3I4JBR4SH3WD131B2")
                .then(res=>{
        ...
        if(this.props.type=="Coin"){
            ...
            axios.get("https://api.bscscan.com/api?module=account&action=txlist&address="+address+"&startblock=0&endblock=9999999999&page=1&offset=100&sort=desc&apikey=S7U2FT2YMJC4HZFA49EKK2SQEGC4UBSVCE")
                .then(res=>{
                    console.log(res.data.result)
    ...
    render() {
        var dval = Array.isArray(this.state.data)? this.state.data.map(res=>{
            //console.log(res)
        ...
        return (
            <>
                {this.state.loading === true ?
                    ...
                    :
                    <>
                        {(dval.length > 0 || tdata.length > 0) ? <>{dval} {tdata}</>:
