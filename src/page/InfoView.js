...
class InfoView extends Component {
    constructor({match}) {
        super();
        this.state={
            id:match.params.id,title:"",des:"",created_at:"",loading:true
        }
    }
    componentDidMount() {
        window.scrollTo(0, 0);
        Axios.get(ApiUrl.baseurl+"info/"+this.state.id)
            .then(res=>{
                if(res.data.error){
                    console.log("err")
                }else{
                    ...
                    if(res.data.length>0){
                        var val = res.data[0]
                        this.setState({title:val["title"],des:val["des"],created_at:val["created_at"],
                            loading:false})
                    }
    ...
    render() {
        return (
            <>
                <RouteCheck/>
                <div style={{width:"100%",minHeight:"800px",height:"auto",background:"#D0E2F1"}}>
                    <title>Notification  {this.state.title}</title>
